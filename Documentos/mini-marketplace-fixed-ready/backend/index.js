...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    res.status(201).json({ user: { id: user.id, name, email, role, photoUrl: user.photoUrl || null }, token });
  } catch (e) {
    console.error(e);
...
app.post('/auth/login', async (req, res) => {
  try {
    ...
    const token = sign(user);
    res.json({ user: { id: user.id, name: user.name, email: user.email, role: user.role, photoUrl: user.photoUrl || null }, token });
  } catch (e) {
...
// === Foto do Prestador (upload base64) ===
...
app.post('/providers/:providerId/photo', auth('PROVIDER'), async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn('Falha ao atualizar photoUrl no banco:', e.message);
    }
